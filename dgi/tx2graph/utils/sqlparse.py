...
def name(s):
    ...
    return (
        (r[0], [])
        if r
        and r[1][0] not in reserved
        and (r[1][0][0] == "_" or r[1][0][0].isalpha() or r[1][0][0] == '"')
        else ()
    )
...
paren = seq(
    op("("), star(seq(until("(", ")"), lambda s: paren(s))), until("(", ")"), op(")")
)

# skip until one of strs occurs, ignoring anything inside parenthesis
until_ignoring_paren = lambda *strs: seq(  # noqa: E0012,E731 pylint: disable=unnecessary-lambda-assignment
...
def comma_sep(e):
    return seq(
        e, star(seq(op(","), e))
    )  # noqa: E0012,E731 pylint: disable=unnecessary-lambda-assignment
...
def ascxt(e):
    return match(  # noqa: E0012,E731 pylint: disable=unnecessary-lambda-assignment
...
callexp = choice(
    seq(
        ...
        callargs,
        op(")"),
        option(seq(op("within"), op("group"), paren)),
    ),
    seq(
        op("cast"),
        op("("),
...
naryexp = choice(seq(op("-"), lambda s: colexp(s)), binaryexp)

colexp = choice(naryexp, colexp0)

colexp1 = match(
    ascxt(colexp),
    lambda s, _: [{split(s[0][0])[1]: s[0]}]
    if len(s) == 1 and isinstance(s[0], tuple) and qname(s[0][0]) == ("", [])
    else s,
)
...
tblexp = choice(
    seq(match(qname), option(seq(op("partition"), op("("), name, op(")")))),
    lambda s: nestedexp(s),
...
condexp = seq(
    choice(
        seq(op("("), lambda s: condexp(s), op(")")),
        seq(op("not"), lambda s: condexp(s)),
        seq(op("exists"), choice(colexp, lambda s: nestedexp(s))),
        seq(
            colexp1,
            choice(
                seq(
        ...
        seq(
            choice(op(":_where_and"), op(":_or"), op(":_and")),
            op("("),
            comma_sep(lambda s: condexp(s)),
            op(")"),
        ),
        seq(op(":_in"), op("("), qname, option(op("=")), variable, op(")")),
        callexp,
    ),
    option(seq(choice(op("and"), op("or")), lambda s: condexp(s))),
)
...
whrexp = choice(
    ...
    seq(
        choice(
            ...
            seq(
                choice(op(":_where_and"), op(":_where_or")),
                op("("),
                option(condexp),
                star(seq(option(op(",")), choice(condexp, variable))),
                op(")"),
            ),
        ),
        option(seq(choice(op("and"), op("or")), condexp)),
    ),
)
...
selexp = withcxt(
    unioncxt(
        seq(
            match(op("select")),
            option(op("distinct")),
            option(seq(op("top"), literal)),
            colsexp,
            option(op(",")),
            option(seq(op("into"), colsexp)),
            op("from"),
            frmspec,
        )
    )
...
updexp = withcxt(
    seq(
        match(op("update")),
        frmcxt(seq(ascxt(match(qname)), op("set"), assignsexp)),
        option(whrexp),
    )
)
insexp = withcxt(
        ...
        frmcxt(
            seq(
                ...
                option(
                    seq(
                        op("("),
                        comma_sep(match(qname, lambda s, v: s + [(v,)])),
                        op(")"),
                    )
                ),
            )
...
if __name__ == "__main__":
    ...
    print(
        sqlexp("select * from ((select * from t) union (select * from d) order by x)")
    )
    ...
    print(
        sqlexp("select t.a, s.a from s, (select * from x union select * from y) as t")
    )
